<#
.SYNOPSIS
    Rotates Event Hub Namespace or Event Hub Instance Access Keys (Primary or Secondary) and securely stores them in Azure Key Vault.
...
.DESCRIPTION
    Rotates the specified Event Hub namespace or Event Hub instance access key (Primary or Secondary) and automatically
    stores the new key value in Azure Key Vault for secure retrieval by applications.

    Key Features:
    - Rotates Primary or Secondary keys for namespaces or Event Hub instances
...
.PARAMETER ResourceGroup
    The name of the Azure Resource Group containing the Event Hub namespace.

.PARAMETER EventHubSubscriptionId
    The Azure Subscription ID containing the Event Hub namespace. Defaults to the current subscription.

.PARAMETER Namespace
    The name of the Event Hub namespace.

.PARAMETER EventHubName
    The name of the specific Event Hub instance (optional). If not specified, rotates namespace-level keys.
...
.EXAMPLE
    ...
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "my-rg" -Namespace "my-namespace" `
        -KeyName "RootManageSharedAccessKey" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -SecretNamePrefix "eventhub" `
        -SecretExpiryDays 90

.EXAMPLE
    # Rotate Event Hub instance keys (interactive)
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "my-rg" -Namespace "my-namespace" `
        -EventHubName "my-event-hub" -KeyName "SendListen" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -SecretNamePrefix "eventhub"
...
.EXAMPLE
    ...
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "my-rg" -Namespace "my-namespace" `
        -KeyName "RootManageSharedAccessKey" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -SecretNamePrefix "eventhub" -Force `
        -SecretExpiryDays 30
...
.EXAMPLE
    ...
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "my-rg" -Namespace "my-namespace" `
        -EventHubName "my-event-hub" -KeyName "SendListen" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -SecretNamePrefix "eventhub" -ShowVerbose

.EXAMPLE
    # Emergency recovery mode (shows full keys on failure)
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "my-rg" -Namespace "my-namespace" `
        -EventHubName "my-event-hub" -KeyName "SendListen" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -SecretNamePrefix "eventhub" -ShowRecoveryKey

.EXAMPLE
    # Cross-subscription rotation (Event Hub and Key Vault in different subscriptions)
    .\Rotate-EventHubAccessKeys.ps1 -ResourceGroup "eventhub-rg" -EventHubSubscriptionId "11111111-1111-1111-1111-111111111111" `
        -Namespace "my-namespace" -EventHubName "my-event-hub" -KeyName "SendListen" -Rotate "Primary" `
        -KeyVaultName "my-keyvault" -KeyVaultResourceGroup "security-rg" -KeyVaultSubscriptionId "22222222-2222-2222-2222-222222222222" `
        -SecretNamePrefix "eventhub" -Force

.NOTES
    Author: Andrew Wood
    Version: 2.1
    Requires: Az.EventHub, Az.KeyVault PowerShell modules
    ...
    - Event Hubs Data Owner (recommended for both namespace and instance operations)
    ...
    - Microsoft.EventHub/namespaces/authorizationRules/write
      OR for Event Hub instance operations:
...
$isNamespaceLevel = [string]::IsNullOrEmpty($EventHubName)
$scopeDescription = if ($isNamespaceLevel) { "Event Hub Namespace" } else { "Event Hub Instance '$EventHubName'" }

Write-Host "=== Event Hub Key Rotation Script ===" -ForegroundColor Cyan
...
Write-Host "Event Hub RG: $ResourceGroup" -ForegroundColor Gray
if ($EventHubSubscriptionId) {
    Write-Host "Event Hub Sub: $EventHubSubscriptionId" -ForegroundColor Gray
}
...
foreach ($module in @("Az.EventHub","Az.KeyVault")) {
    try {
...
if (-not $Force) {
    $targetDescription = if ($isNamespaceLevel) {
        ...
        "Event Hub instance '$EventHubName' in namespace '$Namespace'"
    }
...
Write-Host "`n[3/8] Switching to Event Hub context..." -ForegroundColor Yellow
$eventHubOriginalContext = $null
try {
    if ($EventHubSubscriptionId -and $EventHubSubscriptionId -ne $context.Subscription.Id) {
        Write-Host "  Switching to subscription: $EventHubSubscriptionId" -ForegroundColor Gray
        $eventHubOriginalContext = $context
        Set-AzContext -SubscriptionId $EventHubSubscriptionId -ErrorAction Stop | Out-Null
        Write-Host "  ✓ Switched to Event Hub subscription" -ForegroundColor Green
    } else {
        Write-Host "  ✓ Using current subscription for Event Hub" -ForegroundColor Green
    }
}
catch {
    Write-Error "Failed to switch to Event Hub subscription: $_"
    exit 1
}
...
try {
    if ($isNamespaceLevel) {
        $keys = Get-AzEventHubNamespaceKey `
            -ResourceGroupName $ResourceGroup `
            -Namespace $Namespace `
            -AuthorizationRuleName $KeyName `
            -ErrorAction Stop
    } else {
    ...
    Write-Host "  - Event Hub Namespace '$Namespace' exists" -ForegroundColor Red
    if (-not $isNamespaceLevel) {
        Write-Host "  - Event Hub instance '$EventHubName' exists" -ForegroundColor Red
    }
...
try {
    if ($isNamespaceLevel) {
        $newKeys = New-AzEventHubNamespaceKey `
            -ResourceGroupName $ResourceGroup `
            -Namespace $Namespace `
            -AuthorizationRuleName $KeyName `
...
$secretNameBase = if ($isNamespaceLevel) {
    "$SecretNamePrefix-$Namespace-$KeyName"
} else {
    "$SecretNamePrefix-$Namespace-$EventHubName-$KeyName"
}
...
try {
    ...
    if ($ShowRecoveryKey) {
        ...
        Write-Host "   Alternative: Check the Event Hub portal for the new key value" -ForegroundColor Yellow
    }
...
try {
    ...
    if ($originalContext) {
        if ($eventHubOriginalContext) {
            Write-Host "  Switching back from Key Vault to Event Hub context: $($eventHubOriginalContext.Subscription.Id)" -ForegroundColor Gray
            $targetContext = $eventHubOriginalContext
        } else {
            Write-Host "  Switching back from Key Vault to Event Hub context: $($originalContext.Subscription.Id)" -ForegroundColor Gray
            $targetContext = $originalContext
        }
        Set-AzContext -SubscriptionId $targetContext.Subscription.Id -ErrorAction Stop | Out-Null
        Write-Host "  ✓ Restored Event Hub context" -ForegroundColor Green
    }
    # Restore Event Hub context if needed (legacy logic, should not be needed if above runs)
    if ($eventHubOriginalContext -and -not $originalContext) {
        Write-Host "  Switching back to original subscription: $($eventHubOriginalContext.Subscription.Id)" -ForegroundColor Gray
        Set-AzContext -SubscriptionId $eventHubOriginalContext.Subscription.Id -ErrorAction Stop | Out-Null
        Write-Host "  ✓ Restored original context" -ForegroundColor Green
    }
    if (-not $originalContext -and -not $eventHubOriginalContext) {
...
Write-Host "  • Event Hub Namespace: $Namespace" -ForegroundColor White
if (-not $isNamespaceLevel) {
    Write-Host "  • Event Hub Instance: $EventHubName" -ForegroundColor White
}
